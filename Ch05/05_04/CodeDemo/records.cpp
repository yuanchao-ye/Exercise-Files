...
void StudentRecords::report_card(int sid)
{
	...
	cout << student_name << " takes course " << endl; 
	for (Grade & grd : grades) {
		if (grd.get_student_id() == sid){
			// get course_id
			int course_id = grd.get_course_id();
			std::string course_name = get_course_name(course_id);
			char grade = grd.get_grade();
			cout << "	" << course_name << ", score = " << grade << endl;
			unsigned char current_credits = get_course_credits(course_id);
			credits += current_credits;
			points += get_num_grade(grd.get_grade()) * current_credits;
		}
	}
	// float gpa = get_GPA(sid);
	cout << "The total GPA is : " << gpa << endl;
}
